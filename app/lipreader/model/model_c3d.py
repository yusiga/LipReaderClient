...
class C3dModel(nn.Module):
    def __init__(self, decode_mode):
        super().__init__()

        self.cnn = Cnn3d()
        if decode_mode == "py":
            self.seq2seq = Seq2Seq(vocab_size=417)
        elif decode_mode == "hz":
            self.seq2seq = Seq2Seq(vocab_size=3521)

    def forward(self, video, video_mask, tgt=None):
        length = torch.sum(1 - video_mask.long(), dim=-1).to("cpu")
        video = self.cnn.forward(video)
        if tgt is not None:
            logits, pred = self.seq2seq.forward(video, length, tgt)
        else:
            logits, pred = self.seq2seq.greedy_search(video, length, 30)
        return logits, pred
